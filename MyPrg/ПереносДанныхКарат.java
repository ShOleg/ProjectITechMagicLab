 ...
 * @author ShOleg
 */
public class ПереносДанныхКарат extends SuperAction {
    ...
    private String createNachUd() throws SQLException {
        ...
        pst.setString(1, "Карат-2004");
        pst.setDate(2, date2);
        pst.setDate(3, date1);

        pst.setString(4, "Карат-2004");
        pst.setDate(5, date2);
        pst.setDate(6, date1);

        pst.setString(7, "Карат-2004");
        pst.setDate(8, date2);
        pst.setDate(9, date1);
    ...
    private String createTabel() throws SQLException {
        ...
        if (rs.next()) {
            ...
            do {
                ПереносДанныхУниверсал.insertSostavTabel(codeDoc, rs, "Альфа", "А1");
            } while (rs.next());
        }
    ...
    private void insertDR(final long codeDoc) throws SQLException {
        ...
        pst.setString(9, "Перенос данных для среднего заработка из Карат-2004");
        pst.executeUpdate();
        pst.close();
    }
    ...
    private void insertSostav(final long codeDoc, ResultSet rs) throws SQLException {
        if (!ПереносДанныхУниверсал.isFind("Альфа", "А1" + rs.getString("ТабНомер"))) {
            return;
        }
        ...
        pst.setString(4, "А1" + rs.getString("ТабНомер"));
